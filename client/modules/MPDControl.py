...
def handle(text, speaker, mic, profile):
    ...
    try:
        ...
        speaker.clean_and_say("I'm sorry. It seems that Spotify is not enabled. Please " +
                "read the documentation to learn how to configure Spotify.")
        return

    speaker.clean_and_say("Please give me a moment, I'm loading your Spotify playlists.")
...
class MusicMode(object):
    ...
    def delegateInput(self, input):
        ...
        if "PLAYLIST" in command:
            ...
            self.speaker.clean_and_say("Stopping music")
            self.music.stop()
            return
        elif "PLAY" in command:
            self.speaker.clean_and_say("Playing %s" % self.music.current_song())
            self.music.play()
            return
        elif "PAUSE" in command:
            self.speaker.clean_and_say("Pausing music")
            # not pause because would need a way to keep track of pause/play
            # state
            self.music.stop()
            return
        elif any(ext in command for ext in ["LOUDER", "HIGHER"]):
            self.speaker.clean_and_say("Louder")
            self.music.volume(interval=10)
            self.music.play()
            return
        elif any(ext in command for ext in ["SOFTER", "LOWER"]):
            self.speaker.clean_and_say("Softer")
            self.music.volume(interval=-10)
            self.music.play()
            return
        elif "NEXT" in command:
            self.speaker.clean_and_say("Next song")
            self.music.play()  # backwards necessary to get mopidy to work
            self.music.next()
            self.speaker.clean_and_say("Playing %s" % self.music.current_song())
            return
        elif "PREVIOUS" in command:
            self.speaker.clean_and_say("Previous song")
            self.music.play()  # backwards necessary to get mopidy to work
            self.music.previous()
            self.speaker.clean_and_say("Playing %s" % self.music.current_song())
            return
        ...
        # if songs:
        #     self.speaker.clean_and_say("Found songs")
        ...
        # else:
        #     self.speaker.clean_and_say("No songs found. Resuming current song.")
        #     self.music.play()

        # PLAYLIST SELECTION
        playlists = self.music.fuzzy_playlists(query=command)
        if playlists:
            self.speaker.clean_and_say("Loading playlist %s" % playlists[0])
            self.music.play(playlist_name=playlists[0])
            self.speaker.clean_and_say("Playing %s" % self.music.current_song())
        else:
            self.speaker.clean_and_say("No playlists found. Resuming current song.")
            self.music.play()
        ...
        while True:
            ...
            if input:
                if "close" in input.lower():
                    self.speaker.clean_and_say("Closing Spotify")
                    return
